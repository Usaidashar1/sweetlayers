...
import {
  getStorageConfig, setStorageConfig, clearStorageConfig,
  uploadBlob, fetchProductsFromCloud, CONTAINERS,
} from "@/config/storage";
...
export function AdminPortal() {
  ...
  // Settings
  const [config, setConfigState] = useState(getStorageConfig);
  const [sasInput, setSasInput] = useState(config?.sasToken ?? "");
  ...
  const handleSaveProduct = async () => {
    ...
    if (!config?.sasToken) {
      toast.error("Please configure your Azure SAS token in Settings first");
    ...
    try {
      ...
      if (imageFile) {
        ...
        imageUrl = `https://${STORAGE_ACCOUNT}.blob.core.windows.net/${CONTAINERS.images}/${blobName}`;
      }
  ...
  const handleDelete = async (product: Product) => {
    ...
    if (!config?.sasToken) {
      toast.error("SAS token not configured");
      return;
    }
    try {
  ...
  const saveSettings = () => {
    setStorageConfig({ account: STORAGE_ACCOUNT, sasToken: sasInput.trim() });
    setConfigState({ account: STORAGE_ACCOUNT, sasToken: sasInput.trim() });
      ...
      <div className="min-h-screen bg-stone-100 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full bg-amber-700 hover:bg-amber-800 text-white" onClick={handleLogin}>
              Unlock
            </Button>
          </div>
          <p className="text-xs text-stone-400 text-center mt-4">🔒 This page is restricted to authorized staff only.</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <header className="bg-white border-b border-stone-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant={activeTab === "products" ? "default" : "ghost"} size="sm" onClick={() => setActiveTab("products")} className={activeTab === "products" ? "bg-amber-700 text-white" : ""}>
              Products
            </Button>
            <Button variant={activeTab === "settings" ? "default" : "ghost"} size="sm" onClick={() => setActiveTab("settings")} className={activeTab === "settings" ? "bg-amber-700 text-white" : ""}>
          ...
          {activeTab === "products" && (
            <motion.div key="products" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="flex items-center justify-between mb-6">
                ...
                <Button onClick={() => { resetForm(); setShowProductForm(true); }} className="bg-amber-700 hover:bg-amber-800 text-white gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex gap-1">
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-stone-400 hover:text-amber-700" onClick={() => openEdit(product)}>
                              <Edit3 className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-stone-400 hover:text-red-600" onClick={() => handleDelete(product)}>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-amber-700 font-bold">{CURRENCY}{product.offerPrice?.toFixed(2) ?? product.price.toFixed(2)}</span>
                          {product.offerPrice && <span className="text-xs text-stone-400 line-through">{CURRENCY}{product.price.toFixed(2)}</span>}
                        </div>
            ...
            <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-xl">
              ...
              <div className="bg-white rounded-xl border border-stone-200 p-6 space-y-6">
                <div>
                  ...
                  <p className="text-xs text-stone-400 mt-1">To change this, edit <code>src/config/app.ts</code> and redeploy.</p>
                </div>
                <div>
                  ...
                  <div className="relative mt-1">
                    <Input
                      ...
                      placeholder="Paste your Azure Blob Storage SAS token here..."
                      className="pr-10"
                    />
                    <button onClick={() => setShowSas(!showSas)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600">
                      {showSas ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                  </div>
                  <p className="text-xs text-stone-400 mt-1">This token allows the admin to upload photos and save product data. It is stored locally in your browser.</p>
                </div>
                <Button onClick={saveSettings} className="bg-amber-700 hover:bg-amber-800 text-white gap-2">
                ...
                <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
                  <h4 className="font-semibold text-amber-800 mb-2 flex items-center gap-2"><RefreshCw className="h-4 w-4" /> How to get a SAS Token</h4>
                  <ol className="text-sm text-amber-900 space-y-1 list-decimal list-inside">
                    ...
                    <li>Open your Storage Account → Shared access signature</li>
                    <li>Allowed services: <strong>Blob</strong></li>
                    <li>Allowed resource types: <strong>Object</strong></li>
                    <li>Allowed permissions: <strong>Read, Create, Write</strong></li>
                    <li>Set an expiry date (e.g., 1 year from now)</li>
                    <li>Click <strong>Generate SAS and connection string</strong></li>
                    <li>Copy the <strong>SAS token</strong> (starts with <code>?sv=</code>) and paste above</li>
                  </ol>
                </div>
              </div>
            </motion.div>
      ...
      <Dialog open={showProductForm} onOpenChange={(open) => { if (!open) { setShowProductForm(false); resetForm(); } }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Fill in the details below. Photos are uploaded directly to Azure Blob Storage.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              <div
                className="mt-1 border-2 border-dashed border-stone-300 rounded-xl p-6 text-center hover:border-amber-400 hover:bg-amber-50 transition-colors cursor-pointer"
              ...
              >
                {imagePreview ? (
                  ...
                  <div className="py-8">
                    ...
                    <p className="text-stone-400 text-xs">JPG, PNG, WEBP up to 5MB</p>
                  </div>
                )}
                <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
              </div>
              {imagePreview && (
                <Button variant="ghost" size="sm" className="mt-2 text-red-600" onClick={() => { setImageFile(null); setImagePreview(editingProduct?.image ?? ""); }}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="e.g. Butter Croissant" />
              </div>
              <div>
            ...
            <div>
              ...
              <Textarea value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder="Describe the product..." rows={3} />
            </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <div>
                <Label>Offer Price ({CURRENCY})</Label>
                <Input type="number" step="0.01" value={formOfferPrice} onChange={(e) => setFormOfferPrice(e.target.value)} placeholder="Sale price (optional)" />
              </div>
              <div>
                <Label>Calories</Label>
                <Input type="number" value={formCalories} onChange={(e) => setFormCalories(e.target.value)} placeholder="e.g. 250" />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input value={formTags} onChange={(e) => setFormTags(e.target.value)} placeholder="vegan, organic, best-seller" />
              </div>
              <div>
                <Label>Allergens (comma separated)</Label>
                <Input value={formAllergens} onChange={(e) => setFormAllergens(e.target.value)} placeholder="gluten, dairy, nuts" />
              </div>
            </div>

            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                <Checkbox id="isNew" checked={formIsNew} onCheckedChange={(v) => setFormIsNew(v as boolean)} />
                <Label htmlFor="isNew" className="cursor-pointer">Mark as New</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="isBestseller" checked={formIsBestseller} onCheckedChange={(v) => setFormIsBestseller(v as boolean)} />
          ...
          <div className="flex gap-3 mt-6">
            ...
            <Button className="flex-1 bg-amber-700 hover:bg-amber-800 text-white gap-2" onClick={handleSaveProduct} disabled={saving}>
